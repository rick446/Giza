...
from .util import Bunch, LazyProperty
...
class Migrate(SchemaItem):
    ...
    def obj_to_list(cls, key_name, value_name=None):
        ...
        def migrate_scalars(value):
            return [
                Bunch({ key_name: k, value_name: v})
                for k,v in value.iteritems() ]
        def migrate_objects(value):
            return [
                Bunch(dict(v, **{key_name:k}))
                for k,v in value.iteritems() ]
...
class FormEncode(FancySchemaItem):
    '''Just wraps a FormEncode validator's to_python'''
    ...
    def validate(self, value, **kw):
        try:
            return self._validator.to_python(value, None)
        except formencode.Invalid, inv:
            raise Invalid(inv.msg, inv.value, inv.state,
                          error_list=inv.error_list,
                          error_dict=inv.error_dict)

class Anything(FancySchemaItem):
    'Anything goes - always passes validation unchanged except dict=>Object'

    def validate(self, value, **kw):
        if isinstance(value, dict) and not isinstance(value, Bunch):
            return Bunch(value)
        return value

class Object(FancySchemaItem):
    '''Used for dict-like validation.  Also ensures that the incoming object
    does not have any extra keys
    '''
